...
struct ContentView: View {
    ...
    @State private var editingEntry: CrackEntry?
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle("Foundationwatch")
            .toolbar {
...
struct EntryFormView: View {
    ...
    @State private var stage: String = CrackEntry.stageOptions.first ?? ""
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section("Details") {
                    ...
                    Picker("Stage", selection: $stage) {
                        ForEach(CrackEntry.stageOptions, id: \.self) { Text($0).tag($0) }
                    }
            ...
            .navigationTitle(mode.id == "add" ? "New Crack" : "Edit Crack")
            .toolbar {
    ...
    private func save() {
        switch mode {
        case .add:
            let entry = CrackEntry(title: title, stage: stage, date: date, note: note, photoData: photoData)
            store.add(entry)
        case .edit(var entry):
            entry.title = title
            entry.stage = stage
